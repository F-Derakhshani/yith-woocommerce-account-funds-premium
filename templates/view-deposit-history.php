        ...
        <form method="post">
            <div class="filter_container">
                <select name="filter_deposit_type">
                    <option value="" <?php selected( '', $filter_type ); ?>>Show All</option>
                    <option value="deposit" <?php selected( 'deposit', $filter_type );?>><?php _e('Deposit','yith-woocommerce-account-funds');?></option>
                    <option value="pay" <?php selected( 'pay', $filter_type );?>><?php _e('Payment','yith-woocommerce-account-funds');?></option>
                    <option value="restore" <?php selected( 'restore', $filter_type );?>><?php _e('Funds restored','yith-woocommerce-account-funds');?></option>
                    <option value="remove" <?php selected( 'remove', $filter_type );?>><?php _e('Funds refunded','yith-woocommerce-account-funds');?></option>
                    <option value="admin_op" <?php selected( 'admin_op', $filter_type );?>><?php _e('Admin activity','yith-woocommerce-account-funds');?></option>
                </select>
    ...
    <?php if ( count( $user_log_items ) > 0 ): ?>
        <table class="shop_table shop_table_responsive my_account_orders my_funds_history">
                    ...
                    <?php foreach ( $my_deposit_history as $column_id => $column_name ) :
                        switch ( $column_id ) {
                            ...
                            case 'description':
                                ...
                                if( $order ) {
                                    ...
                                    if( 'pay' === $log->type_operation ) {
                                        ...
                                        $content = sprintf( '<p>%s %s %s %s</p>', __( 'Funds used to purchase', 'yith-woocommerce-account-funds' ), $sub_content, __( 'in order', 'yith-woocomerce-funds' ), $order_link );

                                    }
                                    elseif( 'deposit' === $log->type_operation ) {

                                        $content = sprintf( '<p>%s %s</p>', __( 'Funds credited through order', 'yith-woocommerce-account-funds' ), $order_link );
                            ...
                            case 'deposit-amount':
                                ...
                                $fund_totals += floatval( $log->fund_user );
            ...
            <tr class="fund_totals">
                ...
                <td colspan="1"><?php echo wc_price( apply_filters('yith_show_available_funds', YWF_Log()->get_user_fund_total() ) ); ?></td>
            </tr>
            <?php endif;?>
